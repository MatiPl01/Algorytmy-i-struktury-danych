            ...
            if not used:
                for w in range(n):
                    ...
                    if new_weight < weights[w][1]:
                        ...
                        parents[w][1] = (u, used)  # We jump over v so we skip v

    return weights, parents


def get_path(parents, t, used):
    path = [t]

    entry = parents[t][used]
    while entry:
        t, used = entry
        path.append(t)
        entry = parents[t][used]
...
def jumper(G, s, t):
    weights, parents = dijkstra(G, s, t)
    inf = float('inf')
    used = 0 if weights[t][0] < weights[t][1] else 1
    if weights[t][used] == inf:
        print('NO PATH EXISTS')
        return inf
    print('PATH:', get_path(parents, t, used))
    return weights[t][used]
